...
tripo.forEach(function(d) {
  ...
  var flagCarrier = [true, true , true];
...
function scaterGenGol(){
    this.scatGol = [[days[0] , price[0]]];
    for(var i = 1; i < 590; i++){
        scatGol.push([days[i], price[i]]);
    }
}

function scaterGenTam(){
    this.scatTam = [[days[590] , price[590]]];
    for(var i = 590+1; i < 1021; i++){
        scatTam.push([days[i], price[i]]);
    }
}

function scaterGenAzul(){
    this.scatAzul = [[days[1021] , price[1021]]];
    for(var i = 1021+1; i < days.length; i++){
        scatAzul.push([days[i], price[i]]);
    }
}
...
.attr("fill","steelblue")
.on("click",function(d){ //Para questão 3
      console.log("clicou"+ d.carrier);
      console.log("eh "+ currentColor);
      currentColor = currentColor == "steelblue"? (function(d){ 
        if(d.carrier == "Gol") {
        scatterplot = new Scatterplot(container,widgetID,screenX,screenY,totalWidth2,totalHeight2);
        scatterplot.setData(scatGol);
        };  
        if(d.carrier == "Tam"){       
        scatterplot = new Scatterplot(container,widgetID,screenX,screenY,totalWidth2,totalHeight2);
        scatterplot.setData(scatTam);
        }
        if(d.carrier == "Azul"){    
            scatterplot = new Scatterplot(container,widgetID,screenX,screenY,totalWidth2,totalHeight2);
            scatterplot.setData(scatTam);
